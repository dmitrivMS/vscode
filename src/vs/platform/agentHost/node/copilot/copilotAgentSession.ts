...
export class CopilotAgentSession extends Disposable {
	...
	private _resumeSubagentForEvent(e: { readonly agentId?: string }, message?: Message): void {
		...
		if (this._currentTurn) {
			this._rootTurnIdBySubagentToolCallId.set(parentToolCallId, this._currentTurn.id);
		}
	...
	private _directUsageFor(parentToolCallId: string | undefined, create: boolean): DirectUsageAccumulator | undefined {
		if (!parentToolCallId) {
			return this._currentTurn?.directUsage;
		}
		...
		if (slashCommand?.command === 'compact') {
			try {
				...
				if (typeof usedTokens === 'number') {
					...
					const directTurnTokenTotals = this._currentTurn?.directUsage.tokenTotals;
					const directNanoAiu = this._currentTurn?.directUsage.copilotNanoAiu;
	...
	private _subscribeToEvents(): void {
		...
		this._register(wrapper.onSubagentStarted(e => {
			...
			if (this._currentTurn) {
				this._rootTurnIdBySubagentToolCallId.set(e.data.toolCallId, this._currentTurn.id);
			}
